...
--  $Id: templates_parser-print_tree.adb,v 1.4 2002/10/12 16:49:30 obry Exp $
...
begin
   ...
   case T.Kind is
      when Info =>
         Ada.Text_IO.Put_Line ("[INFO] " & To_String (T.Filename)
                           & Natural'Image (T.Ref));
         declare
            I : Tree := T.I_File;
         begin
            while I /= null loop
               Ada.Text_IO.Put (" -> ");
               Ada.Text_IO.Put_Line (To_String (I.File.Info.Filename));
               I := I.Next;
            end loop;
         end;
      ...
      when If_Stmt  =>
         ...
         Print_Indent (Level);
         Ada.Text_IO.Put_Line ("[END_IF_STMT]");
         Print_Tree (T.Next, Level);

      when Table_Stmt =>
         Ada.Text_IO.Put_Line ("[TABLE_STMT] TERMINATE_SECTIONS="
                           & Boolean'Image (T.Terminate_Sections));
         Print_Tree (T.Sections, Level + 1);
         Print_Indent (Level);
         Ada.Text_IO.Put_Line ("[END_TABLE_STMT]");
         Print_Tree (T.Next, Level);

      when Section_Stmt =>
         Ada.Text_IO.Put_Line ("[SECTION_STMT]");
         Print_Tree (T.Next, Level + 1);
         Print_Tree (T.N_Section, Level);

      when Include_Stmt =>
         Ada.Text_IO.Put_Line ("[INCLUDE_STMT] "
